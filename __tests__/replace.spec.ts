...
describe(replace, () => {
  it("replaces an existing document with a new version", () => {
    ...
    expect(search(index, "quite").map((doc) => doc.id)).toStrictEqual([1]);
    expect(search(index, "even").map((doc) => doc.id)).toStrictEqual([]);

    replace(index, { id: 1, text: "Some even more interesting stuff" });

    expect(search(index, "stuff").map((doc) => doc.id)).toStrictEqual([2, 1]);
    expect(search(index, "quite").map((doc) => doc.id)).toStrictEqual([]);
    expect(search(index, "even").map((doc) => doc.id)).toStrictEqual([1]);
  });
