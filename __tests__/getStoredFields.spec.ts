...
describe(getStoredFields, () => {
  it("returns the stored fields for the given document ID, or undefined if the document is not in the index", () => {
    ...
    expect(getStoredFields(index, 1)).toStrictEqual({
      title: "Divina Commedia",
      text: "Nel mezzo del cammin di nostra vita",
    });
    expect(getStoredFields(index, 2)).toStrictEqual({
      title: "I Promessi Sposi",
      text: "Quel ramo del lago di Como",
    });
    expect(getStoredFields(index, 3)).toBeUndefined();

    discard(index, 1);
    expect(getStoredFields(index, 1)).toBeUndefined();
  });
});
