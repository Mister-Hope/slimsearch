...
describe("tokenization", () => {
  it("splits on multiple contiguous spaces or punctuation characters correctly", () => {
    ...
    expect(tokenize("a  b...c ? d")).toStrictEqual(["a", "b", "c", "d"]);
  });
...
e forse del mio dir poco ti cale`,
    ...
    expect(search(index, "perch")).toHaveLength(0);
  ...
  it("supports non-latin alphabets", () => {
    ...
    expect(search(index, "софия").map(({ id }) => id)).toStrictEqual([1]);
    expect(search(index, "アネモネ").map(({ id }) => id)).toStrictEqual([2]);
    expect(search(index, "τέχνη").map(({ id }) => id)).toStrictEqual([3]);
    expect(search(index, "الرأس").map(({ id }) => id)).toStrictEqual([4]);
    expect(search(index, "123").map(({ id }) => id)).toStrictEqual([5]);
  });
});
